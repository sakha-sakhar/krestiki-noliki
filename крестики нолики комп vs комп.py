        ...
        for j in range(SIZE):
            ...
            for x in range(1, WIN):
                for b in seqs:
                    ...
                    if player == 2:
                        streak1 = reverse_1_2(streak1)
                        streak1 = reverse_1_2(streak2)
...
def main():
    ...
    victory = 0
    hodov = 0
    board = change_cell_state(board, [random.randint(0, SIZE - 1), random.randint(0, SIZE - 1)], 2)
    while running:
        pygame.display.flip()
        ...
        draw_board(board)
        if not victory:
            board = change_cell_state(board, computer(board, 1), 1)
            hodov += 1
            victory = check_victory(board)
            if not victory:
                board = change_cell_state(board, computer(board, 2), 2)
                hodov += 1
                victory = check_victory(board)
        if victory:
            running = False
...
if __name__ == '__main__':
    ...
    while True:
        main()
    """for koef1 in range(2, 5):
        for koef2 in range(1, 5):
            hodi = 0
            for i in range(TRY):
                hodi += main(koef1, koef2)
            print(f'Коэффициенты: {koef1}, {koef2}')
            print('Ходов в среднем: ' , hodi / TRY)"""
    pygame.quit()
